...
class TestRelionSubtomoAverage(CetsRelionTest):
    def test_instantiate_subtomo_average_from_refine3D(self):
        self.setup_dirs(12)
        sta = RelionSubtomoAverage("Refine3D/job012/run_class001.mrc", "TS_03")
        assert sta.__dict__ == {
    ...
    def test_instantiate_subtomo_average_from_class3D(self):
        self.setup_dirs(17)
        sta = RelionSubtomoAverage("Class3D/job017/run_it025_class001.mrc", "TS_03")
        assert sta.__dict__ == {
            "file_name": "Class3D/job017/run_it025_class001.mrc",
            "particles": "Class3D/job017/run_it025_data.star",
            "path": Path("Class3D/job017/run_it025_class001.mrc"),
            "tomos": "Denoise/job008/tomograms.star",
            "tomo_name": "TS_03",
        }
    ...
    def test_instantiate_subtomo_average_from_reconstruct(self):
        self.setup_dirs(21)
        sta = RelionSubtomoAverage("Reconstruct/job021/merged.mrc", "TS_03")
        assert sta.__dict__ == {
    ...
    def test_instantiate_subtomo_average_from_postprocess(self):
        self.setup_dirs(49)
        sta = RelionSubtomoAverage("PostProcess/job049/postprocessed.mrc", "TS_03")
        assert sta.__dict__ == {
    ...
    def test_get_cets_average_from_refine(self):
        ...
        assert len(avg.particle_maps) == 4661
    ...
    def test_get_cets_average_from_class3D(self):
        ...
        sta = RelionSubtomoAverage("Class3D/job017/run_it025_class001.mrc", "TS_03")
        avg = sta.get_cets_average()
        assert len(avg.particle_maps) == 3541
        assert avg.particle_maps[0] == FROM_CL3D

    def test_get_cets_average_from_reconstruct(self):
        self.setup_dirs(21)
        sta = RelionSubtomoAverage("Reconstruct/job021/merged.mrc", "TS_03")
        avg = sta.get_cets_average()
        assert len(avg.particle_maps) == 1871
    ...
    def test_get_cets_average_from_postprocess(self):
        ...
        sta = RelionSubtomoAverage("PostProcess/job049/postprocessed.mrc", "TS_03")
        avg = sta.get_cets_average()
        assert len(avg.particle_maps) == 1826
